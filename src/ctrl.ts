...
export default class PrepCtrl {
  ...
  makeCgOpts = (): CgConfig => {
    ...
    console.log('comments', this.trainingPath?.at(-1)?.data.comments);

    const targetSan = this.trainingPath?.at(-1)?.data.san;
    const uci = calcTarget(fen, targetSan!);

    console.log(this.trainingPath?.at(-2)?.data.fen);
    ...
    console.log('cgconfig', config);
    // console.log('srs', this.chessSrs.path()?.at(-2)?.data.fen);
  ...
  handleLearn = () => {
    ...
    if (!this.getNext()) {
      ...
      this.path = this.trainingPath!.map((p) => p.data.fen);
