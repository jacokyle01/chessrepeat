...
const ImportIntoChapterModal: React.FC = (chapterIndex: number) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    e.preventDefault();
    // TODO other flags, like save comments..
    const pgn = pgnRef.current?.value || '';
    importIntoChapter(chapterIndex, pgn);
    setShowModal(false);
  };
  ...
  return (
    <dialog
      open
      className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-70 border-none bg-white rounded-lg shadow-lg w-full max-w-lg"
    >
      {/* Close button */}
      <button
...
export default ImportIntoChapterModal;
